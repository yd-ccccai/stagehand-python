...
class SimpleModelClient:
    ...
    async def complete(self, 
        ...
        """
        if litellm is None:
            raise ImportError("LiteLLM is required for completion. Install with: pip install litellm")
            
        # Ensure LiteLLM has an API key
        api_key = os.getenv("OPENAI_API_KEY") or os.getenv("MODEL_API_KEY")
        if not api_key:
            raise ValueError("OPENAI_API_KEY or MODEL_API_KEY environment variable must be set")
...
def setup_environment():
    ...
    try:
        ...
        print("WARNING: LiteLLM not installed. LOCAL mode requires it for model inference.")

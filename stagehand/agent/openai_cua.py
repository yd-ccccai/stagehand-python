...
class OpenAICUAClient(AgentClient):
    ...
    def _process_provider_response(
        ...
        computer_call_items = [
            item for item in output_items if item.type == "computer_call"
        ]
        ...
        if (
            reasoning_items
            and reasoning_items[0].summary
            and isinstance(reasoning_items[0].summary, list)
            and len(reasoning_items[0].summary) > 0
        ):
        ...
        if (
            message_items
            and message_items[0].content
            and isinstance(message_items[0].content, list)
        ):
            final_model_message_parts = [
                content_item.text
                for content_item in message_items[0].content
        ...
        if not computer_call_items:
            ...
            self.logger.info(
                f"OpenAI CUA: Task appears complete. Reason: {task_complete_reason}",
                category="agent",
            )
        ...
        self.last_openai_tool_calls = [
            computer_call
        ]  # Store the call for feedback, specifically its `call_id`

        # Defensive check for computer_call.action type
        if not hasattr(computer_call, "action") or not isinstance(
            computer_call.action, BaseModel
        ):
            self.logger.error(
                f"OpenAI computer_call.action is not a Pydantic model or is missing. Type: {type(computer_call.action if hasattr(computer_call, 'action') else None)}. Content: {computer_call.action if hasattr(computer_call, 'action') else 'N/A'}",
                category="agent",
            )
            ...
            return (
                ...
                reasoning_text,
                True,
                f"Error: Invalid action structure from OpenAI for computer_call: {computer_call.id if hasattr(computer_call, 'id') else 'Unknown ID'}",
            )
    ...
    def _format_action_feedback(
        ...
        if (
            not self.last_openai_tool_calls
            or not self.last_openai_tool_calls[0].call_id
        ):
            self.logger.error(
                "Missing last_openai_tool_calls or call_id for formatting action feedback.",
                category="agent",
            )
                    ...
                    "content": [
                        {
                            "type": "output_text",
                            "text": (
                                "Error: Could not associate action feedback with a tool call."
                            ),
                        }
                    ],
                }
        ...
        if action_result["success"]:
            output_content = self.format_screenshot(new_screenshot_base64)
        else:
            # For errors, send an InputText object as the content of the 'output' field.
            # This aligns with the expectation that 'output' should contain a CUA input primitive.
            error_message = (
                f"Action failed on page: {action_result.get('error', 'Unknown error')}"
            )
            self.logger.info(
                f"Formatting failed action feedback for OpenAI: {error_message}",
                category="agent",
            )
            output_content = {"type": "input_text", "text": error_message}
    ...
    async def run_task(
        ...
        for step_count in range(self.max_steps):
            ...
            if agent_action:
                ...
                feedback_items = self._format_action_feedback(
                    action_result, current_screenshot_b64
                )
                current_input_items.extend(
                    feedback_items
                )  # Add feedback for the action to the input list
            ...
            if task_completed:
                ...
                return AgentResult(
                    success=True,
                    actions=[
                        act.action for act in actions_taken if act.action
                    ],  # Extract AgentActionType
                    message=final_model_message or "Task completed.",
                    result=final_model_message
                    or "Task completed.",  # Populate result field
                    completed=True,
                    usage={
            ...
            if not agent_action and not task_completed:
                ...
                return AgentResult(
                    ...
                    result="Model did not provide further actions.",  # Populate result field
                    completed=False,
                    usage={
                        "input_tokens": total_input_tokens,
                        "output_tokens": total_output_tokens,
        ...
        return AgentResult(
            ...
            result="Max steps reached.",  # Populate result field
            completed=False,
            usage={
                "input_tokens": total_input_tokens,
                "output_tokens": total_output_tokens,

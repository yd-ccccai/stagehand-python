...
class AnthropicCUAClient(AgentClient):
    def __init__(
        ...
        **kwargs,
        ...
        self.logger.info(
            f"AnthropicCUAClient initialized for model: {model}. (Full implementation pending)",
            category="agent",
        )
        self.max_steps = (
    ...
    async def run_task(
        ...
        self.logger.warning(
            "AnthropicCUAClient run_task is not fully implemented.", category="agent"
        )
        if self.handler:
    ...
    def _process_provider_response(
        ...
        return (
            None,
            "Stubbed reasoning from Anthropic.",
            True,
            "Stubbed completion message from Anthropic.",
        )
    ...
    def _format_action_feedback(
        ...
        return [
            {
                "role": "user",
                "content": [{"type": "text", "text": "Action feedback stub."}],
            }
        ]
    ...
    def key_to_playwright(self, key: str) -> str:
        self.logger.debug(
            f"AnthropicCUAClient key_to_playwright called for key: {key} (stub)",
            category="agent",
        )
        return key

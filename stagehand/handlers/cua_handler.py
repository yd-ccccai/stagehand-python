...
class CUAHandler:  # Computer Use Agent Handler
    ...
    async def inject_cursor(self) -> None:
        ...
        self.logger.info("Attempting to inject cursor via window.__stagehandInjectCursor", category=StagehandFunctionName.AGENT)
        try:
            await self.page.evaluate("window.__stagehandInjectCursor()")
            self.logger.info("Cursor injection via JS function initiated.", category=StagehandFunctionName.AGENT)
        except Exception as e:
            self.logger.error(f"Failed to call window.__stagehandInjectCursor: {e}", category=StagehandFunctionName.AGENT)

    async def _update_cursor_position(self, x: int, y: int) -> None:
        """Update the cursor position on the page by calling the JS function."""
        try:
            await self.page.evaluate(f"window.__stagehandUpdateCursorPosition({x}, {y})")
        except Exception as e:
            self.logger.warning(f"Failed to call window.__stagehandUpdateCursorPosition: {e}", category=StagehandFunctionName.AGENT)
    ...
    async def _animate_click(self, x: int, y: int) -> None:
        ...
        try:
            ...
            self.logger.warning(f"Failed to call window.__stagehandAnimateClick: {e}", category=StagehandFunctionName.AGENT)

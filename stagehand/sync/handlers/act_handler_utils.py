...
class MethodHandlerContext:
    ...
    xpath: str
    args: list[Any]
    stagehand_page: "SyncStagehandPage"
    initial_url: str
    logger: Callable = None
    dom_settle_timeout_ms: Optional[int] = None


def scroll_to_next_chunk(ctx: MethodHandlerContext) -> None:
    ctx.logger.debug(
    ...
    try:
        ctx.stagehand_page._page.evaluate(
            """
            ({ xpath }) => {
...
def scroll_to_previous_chunk(ctx: MethodHandlerContext) -> None:
    ctx.logger.debug(
...
def scroll_element_into_view(ctx: MethodHandlerContext) -> None:
    ctx.logger.debug(
...
def scroll_element_to_percentage(ctx: MethodHandlerContext) -> None:
    ctx.logger.debug(
...
def fill_or_type(ctx: MethodHandlerContext) -> None:
    try:
        ctx.locator.fill("", force=True)
        text = str(ctx.args[0]) if ctx.args and ctx.args[0] is not None else ""
        ctx.locator.fill(text, force=True)
    except Exception as e:
        ctx.logger.error(
...
def press_key(ctx: MethodHandlerContext) -> None:
    try:
        key = str(ctx.args[0]) if ctx.args and ctx.args[0] is not None else ""
        ctx.locator._page.keyboard.press(key)
...
def click_element(ctx: MethodHandlerContext) -> None:
    ctx.logger.debug(
    ...
    try:
        ...
        ctx.locator.evaluate("(el) => el.click()")
        handle_possible_page_navigation(
            "click",
            ctx.xpath,
            ctx.initial_url,
...
def fallback_locator_method(ctx: MethodHandlerContext) -> None:
    ctx.logger.debug(
...
def handle_possible_page_navigation(
    ...
    try:
        with stagehand_page.context.expect_page(timeout=1500) as new_page_info:
        ...
        new_opened_tab = new_page_info.value
    except Exception:
        new_opened_tab = None
    ...
    if new_opened_tab:
        ...
        new_opened_tab.close()
        stagehand_page._page.goto(new_tab_url)
        stagehand_page._page.wait_for_load_state("domcontentloaded")

    try:
        stagehand_page._wait_for_settled_dom(dom_settle_timeout_ms)
    except Exception as e:
        logger.warning(

...
async def run_all_evals(only_observe=True, model_name=None, specific_eval=None):
    ...
    if specific_eval:
        if specific_eval.startswith("observe_"):
            ...
            if "observe" in specific_eval:
                ...
                print(f"Warning: Could not determine directory for {specific_eval}, will search all allowed directories")
            
            # Add .py extension if needed
            specific_file = f"{specific_eval}.py" if not specific_eval.endswith(".py") else specific_eval
    ...
    for root, _, files in os.walk(base_path):
        ...
        for file in files:
            ...
            if file.endswith(".py") and file not in ("__init__.py", "run_all_evals.py", "test_observe.py"):
    ...
    if not os.getenv("OPENAI_API_KEY") and not os.getenv("MODEL_API_KEY"):
        print("WARNING: Neither OPENAI_API_KEY nor MODEL_API_KEY environment variables are set")
    ...
    if not os.getenv("BROWSERBASE_API_KEY") or not os.getenv("BROWSERBASE_PROJECT_ID"):
        print("Running in LOCAL mode (no BROWSERBASE_API_KEY or BROWSERBASE_PROJECT_ID set)")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--all", action="store_true", help="Run all eval types (not just observe)")
    parser.add_argument("--eval", type=str, help="Run a specific eval by name (e.g., observe_taxes)")
    ...
    for module, res in final_results.items():
        ...
        error = f": {res.get('error')}" if not res.get("_success") and "error" in res else ""
